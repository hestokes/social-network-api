...
const userController = {
    ...
    User.findOne({ _id: params.userId })
      ...
      .then((dbUserData) => {
        if (!dbUserData) {
          res
            .status(404)
            .json({ message: "I did not come across a user with this id!" });
          return;
        }
        res.json(dbUserData);
      })
  ...
  deleteUser({ params }, res) {
    User.findOneAndDelete({ _id: params.userId })
      .then((dbUserData) => {
        if (!dbUserData) {
          res.status(404).json({ message: "No sign of a user with this id!" });
          return;
        }
        res.json(dbUserData);
      })
    ...
    User.findOneAndUpdate(
      ...
      .then((dbUserData) => {
        if (!dbUserData) {
          res
            .status(404)
            .json({ message: "I had no luck finding a user with this id!" });
          return;
        }
        res.json(dbUserData);
      })
